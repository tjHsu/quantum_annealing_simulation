...
int main(int argc, char* argv[]){
  ...
  //set up the annealing time and coupling factor
  ...
  //measure the time for running the whole program
  time_t start_all=time(0);
  clock_t t_all;
  t_all = clock();

  //set up some flags for the function that will only be calculate once
  int env_on=1;
  int readin_psi_on=1;
  //set random is the function that prepare a random state for random_wavef_run(). should be here if readin_psi_on=1
  test.set_random(8);
  //start runing the man process. loop along with total annealing time and coupling factor J.
  for (int i = 0; i < 10; i++) {
